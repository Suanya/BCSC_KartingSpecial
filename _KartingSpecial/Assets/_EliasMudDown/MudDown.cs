...
namespace KartGame.KartSystems
{
    public class MudDown : MonoBehaviour
    {
        ...
        public bool m_hitMud;// elias added

        public ParticleSystem m_mudVFX;//elias added

        [System.Serializable]
        public class StatPowerup
        {
            public MudDown.Stats modifiers;
        ...
        void Awake()
        {
            ...
            if (m_mudVFX != null)//elias added
            {
                AddMudToWheel(RearLeftWheel, -DriftSparkHorizontalOffset, -DriftSparkRotation);
                AddMudToWheel(RearRightWheel, DriftSparkHorizontalOffset, DriftSparkRotation);
            }
        ...
        void AddMudToWheel(WheelCollider wheel, float horizontalOffset, float rotation)//elias added
        {
            if (m_hitMud == true)
            {
                m_hitMud = false;
                GameObject vfx = Instantiate(m_mudVFX.gameObject, wheel.transform, false);
                ParticleSystem spark = vfx.GetComponent<ParticleSystem>();
        ...
        public void MoveVehicle(bool accelerate, bool brake, float turnInput)
        {
            ...
            Debug.Log("MoveVehicle topSpeed is: " + m_FinalStats.TopSpeed);//elias added
            Debug.Log("MoveVehicle Acceleration is  : " + m_FinalStats.Acceleration);//elias added
        ...
        void OnTriggerStay(Collider other)//elias added
        {
            if (other.CompareTag("Mud"))
            {
                ...
                m_audioSource.clip = m_mudClip;
                m_audioSource.Play();
                //Debug.Log("mudClip" + m_mudClip);

                m_FinalStats.TopSpeed = 3;
                m_FinalStats.Acceleration = 2;
                //Debug.Log("TopSpeed" + m_FinalStats.TopSpeed);
                m_hitMud = true;
                MoveVehicle(false, false, 0);
                //Debug.Log("movevehicle");

                DriftGrip = 10;
                GameObject vfx = Instantiate(m_mudVFX.gameObject);
                //ParticleSystem spark = vfx.GetComponent<ParticleSystem>();
